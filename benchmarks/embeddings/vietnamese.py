"""
Vietnamese Embedding (BGE-M3 based) provider.

This module implements the EmbeddingProvider interface for the
AITeamVN/Vietnamese_Embedding_v2 model (fine-tuned BGE-M3), supporting both
...
class VietnameseEmbedding:
    """
    Core Vietnamese embedding model (BGE-M3 based).
...
class VietnameseProvider(EmbeddingProvider):
    """
    Vietnamese embedding provider (BGE-M3 based).

    Implements the EmbeddingProvider interface for the
    AITeamVN/Vietnamese_Embedding_v2 model.
    """
    ...
    def __init__(
        ...
        self._device = device
        self._model: Optional[VietnameseEmbedding] = None
    ...
    def _load_model(self) -> None:
        ...
        if self._model is None:
            logger.info(f"Loading Vietnamese model: {self._model_name}")
            self._model = VietnameseEmbedding(
    ...
    def encode(
        ...
        """Encode texts using the Vietnamese model."""
        self._load_model()
